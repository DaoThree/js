...
async function getChats(user) {
            ...
            if (chat.__x_id._serialized == user) {
                ...
                for (var msg of chat.msgs._models) {
                    if (msg.__x_type && msg.__x_type != "gp2") {
                        chats.push({
                            msg: msg.__x_body,
                            type: (msg.__x_type == "ptt") ? "audio" : msg.__x_type,
                            me: msg.__x_isSentByMe /*, n: msg.__x_isNewMsg, t: msg.__x_t*/
                        });
                    }
                }
                break;
    ...
    alert(JSON.stringify({
        command: "get_chats",
        chats: chats,
        connection: document.getElementsByTagName("canvas")[0] == null
    }));
...
async function getMedia(msg, user, index) {
    ...
    alert(JSON.stringify({
        command: "get_media",
        media: media,
        type: msg.__x_mimetype ? msg.__x_mimetype.split("/")[1].split(";")[0] : "",
        user: user,
        index: index
    }));
}
...
async function getRecentMessages(max) {
    ...
    for (var chat of window.Store.Chat._models) {
        if (chat.__x_id._serialized && chat.__x_formattedTitle) {
            ...
            if (last_msg.__x_type || last_msg.__x_type != "gp2") {
                var contact = {
                    u: chat.__x_id._serialized,
                    n: chat.__x_formattedTitle,
                    c: chat.__x_unreadCount + "",
                    m: last_msg.__x_body || "",
                    t: last_msg.__x_t ? (new Date(last_msg.__x_t * 1000)).toLocaleString() : ""
                };
